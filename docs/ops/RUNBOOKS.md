...
## Runbook: Всплеск 5xx ошибок (P0)

### Симптомы

- **Алерт:** 5xx spike > 1% за 5 минут по Gateway или сервису
- **Симптомы пользователей:** Не могут загрузить данные, видят ошибки
...
2. **Проверить логи Cloudflare Workers:**
   - Cloudflare Dashboard → Workers → `{service-name}` → Logs
...
5. **Проверить X-Version header:**
   - Найти git SHA из алёрта
   - Проверить что было задеплоено в этом коммите
...
2. Проверить connection string в переменных окружения Cloudflare Workers
...
4. Использовать rollback если критично
...
1. Отключить тяжёлые списки (уменьшить limit в пагинации)
2. Включить агрессивное кэширование (увеличить TTL)
3. Отключить не критичные endpoints (аналитика, webhook'и)
...
- **Если критичная проблема** → выполнить rollback последнего деплоя
...
## Runbook: Рост p95 латентности (P0)

### Симптомы

- **Алерт:** Latency p95 > 300ms для публичных GET за 5-10 минут
...
1. **Проверить метрики Cloudflare:**
   ...
   - Проверить latency по методам (GET vs POST)

2. **Проверить метрики Neon БД:**
   - Neon Dashboard → Metrics
   - Проверить query time (p50, p95, p99)
...
4. Рассмотреть увеличение ресурсов Neon
...
**Временная деградация:**
1. Отключить тяжёлые операции (сложные фильтры, большие списки)
2. Включить агрессивное кэширование
3. Упростить логику обработки
...
## Runbook: Падение Clerk JWKS (P0)

### Симптомы

- **Алерт:** Auth Service `/ready` возвращает ошибку Clerk JWKS
- **Симптомы пользователей:** Проблемы с авторизацией/регистрацией
- **Метрики:** Высокий процент ошибок 401/403
...
2. **Проверить доступность Clerk JWKS:**
   ```bash
   curl https://{clerk-domain}.clerk.accounts.dev/.well-known/jwks.json
   # Должен вернуть JSON с ключами
   ```

3. **Проверить логи Auth Service:**
   - Cloudflare Dashboard → Workers → `go2asia-auth-service` → Logs
   - Искать ошибки при загрузке JWKS
   - Проверить `X-Request-Id` для трассировки

4. **Проверить статус Clerk:**
   - Clerk Dashboard → Settings → API Keys
   - Проверить статус Clerk (status.clerk.com)
   - Проверить webhook'и → Logs

5. **Проверить переменные окружения:**
   - Проверить `CLERK_PUBLISHABLE_KEY` в Cloudflare Workers
...
3. Временно отключить проверку JWKS в `/ready` (только для диагностики, не для production)

**Если проблема с ключами:**
1. Проверить правильность `CLERK_PUBLISHABLE_KEY`
2. Проверить формат ключа
3. Обновить ключи если необходимо

**Если проблема с сетью:**
1. Проверить доступность Clerk из Cloudflare Workers
2. Проверить firewall правила
3. Проверить DNS

**Временная деградация:**
1. Использовать кэшированные JWKS ключи
2. Увеличить TTL для кэша JWKS
3. Отключить строгую проверку (только для диагностики)

### Эскалация

- **Если Clerk недоступен > 30 минут** → связаться с поддержкой Clerk
- **Если критичная проблема** → рассмотреть временное отключение проверки JWKS
...
## Runbook: Таймауты к сервисам (P0)

### Симптомы

- **Алерт:** Timeouts/Retries > N за 5-10 минут по прокси-метрикам Gateway
- **Симптомы пользователей:** Запросы зависают или завершаются ошибкой
- **Метрики:** Высокий процент таймаутов в Gateway

### Диагностика

1. **Проверить статус целевого сервиса:**
   ```bash
   curl https://{service}.go2asia.space/health
   curl https://{service}.go2asia.space/ready
   ```

2. **Проверить логи Gateway:**
   - Cloudflare Dashboard → Workers → `go2asia-api-gateway` → Logs
   - Искать запросы с таймаутами
   - Проверить `X-Request-Id` для трассировки
   - Фильтр: timeout = true, последние 10 минут

3. **Проверить метрики Neon БД:**
   - Neon Dashboard → Metrics
   - Проверить query time
   - Проверить количество активных подключений
   - Проверить наличие блокировок

4. **Проверить настройки timeout в Gateway:**
   - Проверить `PROXY_TIMEOUT` в переменных окружения
   - Проверить логику retry
   - Проверить не слишком ли короткий timeout

### Решение

**Если сервис недоступен:**
1. Проверить статус Cloudflare Workers
2. Проверить статус Neon БД
3. Проверить connection string
4. Выполнить rollback последнего деплоя если проблема началась после деплоя

**Если проблема в БД:**
1. Проверить медленные запросы в Neon Dashboard
2. Оптимизировать запросы
3. Проверить connection pooling
4. Рассмотреть увеличение ресурсов

**Если проблема в timeout:**
1. Увеличить `PROXY_TIMEOUT` для критичных endpoints
2. Оптимизировать запросы к сервисам
3. Добавить retry с exponential backoff

**Временная деградация:**
1. Отключить тяжёлые операции
2. Упростить логику обработки
3. Использовать кэш вместо прямых запросов к БД

### Эскалация

- **Если не решено за 15 минут** → связаться с DevOps командой
- **Если критичная проблема** → выполнить rollback
